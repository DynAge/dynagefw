...
from .utils import get_info_dict, flatten_dict, compare_info_dicts, nest_dict, prepare_info_dict, clean_nan, \
    load_tabular_file
...
def get_flat_info(fw, project_id, subject_name, session_name, level):
    ...
    """
    session = upload_bids.handle_session(fw, project_id, session_name, subject_name)
    info_nested = get_info_dict(level, fw.get_session(session.id))
    info_flat = flatten_dict(info_nested)
    return info_flat


def upload_info(fw, project_id, subject_name, session_name, level, info_flat, update_values=False):
    """
    ...
    current_info_flat = get_flat_info(fw, project_id, subject_name, session_name, level)
    diff, new = compare_info_dicts(current_info_flat, clean_nan(info_flat))
    if diff and not update_values:
        raise Exception("update_values is False but you want to upload changed values {} {} {} {}" \
    ...
    if new or diff:
        ...
        logger.info("New or changed values found for {} {}. "
                    "Uploading. {}".format(subject_name, session_name, info_flat_upload))
        logger.debug("{}".format(info_flat_upload))

        info_flat_upload = clean_nan(info_flat_upload)  # replace np.nan with "", otherwise the db won't take them
        info_nested = nest_dict(info_flat_upload)
        info = prepare_info_dict(level, info_nested)
        session = upload_bids.handle_session(fw, project_id, session_name, subject_name)
        fw.modify_session(session.id, info)
    else:
        logger.info("No new or changed values found. Do nothing. {} {}".format(subject_name, session_name))


def upload_tabular_file(fw, filename, project_id, update_values=False):
    df = load_tabular_file(filename)

    for i, row in df.iterrows():
        subject_name = row.subject
        session_name = row.session
        info = row.drop(["subject", "session"]).to_dict()
        ...
        if session_name:
            ...
            if not session:
                raise Exception("Cannot find session for subject {}. Before uploading to the subject tab, "
                                "create at least one session for this subject.".format(subject_name))
            session_name = session["label"]
            level = "subject"

        logger.info("Preparing {} {} for {} level".format(subject_name, session_name, level))
        logger.debug("Uploading {}".format(info))

        upload_info(fw, project_id, subject_name, session_name, level, info, update_values)
...
def get_fw_api(api_key=None):
    ...
    if not api_key:
        raise Exception("No flywheel api key provided. Either pass explicitly, or set an env var FWAPI")
    return api_key
...
def upload_tabular_file_wrapper(filename, project_label, group_id, api_key=None, create=False, raise_on=None,
    ...
    upload_tabular_file(fw, filename, project_id, update_values)
